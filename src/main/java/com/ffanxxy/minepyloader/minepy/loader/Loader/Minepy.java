...
public class Minepy {
     ...
     */
        ...
        public void run(List<Argument> arguments) {
            statements.runWithArg(new ArrayList<>());
        }
    ...
    public Minepy(Script script) {
         ...
         * 全局变量
         */
        Map<AccessAndIndex, Variable<?>> GlobalVariables = Variable.GlobalVariables;
        // 变为Method
        for(Line line : readLines) {
            ...
            if(line.i == 0) {
                ...
                StatementManager manager = new StatementManager(line.line);
    ...
    public void runStatic() {
        METHODS.stream().filter(
                method -> method.path.isPack(this.path)
    ...
    public static class MethodParser implements Parser<MethodDefiner> {
        ...
        public MethodParser(String string) {
            ...
            AccessModifiers accessModifiers;
            if(words.size() == 3) {
                accessModifiers = AccessModifiers.fromName(words.get(2));
            } else if(words.size() == 2) {
                accessModifiers = AccessModifiers.DEFAULT;
            } else {
                throw new UnexpectedMethodException("Can't parser method with: " + string);
            }

            method = new MethodDefiner(
                    accessModifiers,
                    words.get(0),
                    DataType.fromName(words.get(1)),
    ...
    public static interface RunnableBlock {
        void run(Map<ScopeAndName, Variable<?>> variableMap);
    }
    ...
    public static class Statement implements RunnableBlock {
        ...
        @Override
        public void run(Map<ScopeAndName, Variable<?>> variableMap) {
            node.runWithArg(variableMap);
        }
    }
    ...
    public static class Statements implements RunnableBlock {
        ...
        public void runWithArg(List<Argument> arguments) {
